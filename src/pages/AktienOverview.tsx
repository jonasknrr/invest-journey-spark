      ...
      <motion.div className="px-5 space-y-5" variants={containerVariants} initial="hidden" animate="visible">
        ...
        <motion.div variants={itemVariants} className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm font-body">SMI & DJIA Aktien kaufen</p>
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="flex gap-3">
          ...
          <div className="flex-1 rounded-3xl bg-[hsl(var(--level-aktien)/0.08)] border border-[hsl(var(--level-aktien)/0.15)] shadow-card p-4">
            ...
            <p className="font-display text-2xl font-bold text-[hsl(var(--level-aktien))] tabular-nums">
              {aktienTotal.toLocaleString('de-CH')} $
            </p>
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants}>
          <h2 className="font-display text-lg font-bold text-foreground mb-3">Aktien</h2>
        </motion.div>
              ...
              return (
                ...
                >
                  ...
                  <div className="w-12 h-12 rounded-2xl bg-[hsl(var(--level-aktien)/0.12)] flex items-center justify-center flex-shrink-0">
                    <TrendUp size={22} weight="fill" className="text-[hsl(var(--level-aktien))]" />
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      <span className="text-xs text-muted-foreground font-body">{stock.ticker}</span>
                      <span className="text-[10px] text-muted-foreground/60 font-body">•</span>
                      <span className="text-[10px] text-muted-foreground font-body">{stock.market}</span>
                    </div>
                  </div>
        ...
        {aktienTotal > 0 && (
          <motion.div variants={itemVariants} className="rounded-3xl bg-primary/8 border border-primary/15 p-4">
            <p className="font-body text-sm text-primary leading-relaxed">
              💡 Du hast insgesamt {aktienTotal.toLocaleString('de-CH')} $ in Aktien investiert.
            </p>
          </motion.div>
        )}
      </motion.div>
