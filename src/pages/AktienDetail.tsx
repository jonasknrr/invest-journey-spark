...
const AktienDetail = () => {
  ...
  const [stock, setStock] = useState<StockListItem | null>(null);

  useEffect(() => {
    getAllStocks().then((list) => {
      setStock(list.find((s) => s.ticker === ticker) ?? null);
    });
  }, [ticker]);

  const currentAmount = ticker ? getProductAmount('aktien', ticker) : 0;
  ...
  const handleConfirm = () => {
    ...
    setProductAmount('aktien', ticker, isReset ? 0 : numericValue);
    navigate(`/challenge/${levelId}/aktien`);
  };
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="px-5 pt-2 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm font-body">{stock.ticker} · {stock.market}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6">
        ...
        {quickAmounts.length > 0 && (
          <div className="flex gap-2 mt-6">
            ...
            <button
              ...
              className="h-10 px-5 rounded-2xl bg-[hsl(var(--level-aktien)/0.12)] text-[hsl(var(--level-aktien))] font-display font-bold text-sm hover:opacity-80 transition-colors"
            >
              Max
            </button>
          </div>
        ...
        <input
          ...
          className="mt-6 w-full max-w-xs h-14 rounded-2xl border-2 border-border bg-card text-center font-display text-lg font-bold text-foreground focus:border-[hsl(var(--level-aktien))] focus:outline-none transition-colors tabular-nums"
        />
      </div>
