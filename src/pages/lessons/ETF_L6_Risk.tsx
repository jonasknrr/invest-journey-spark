...
const sortEvents: SortEvent[] = [
  { emoji: '💼', title: 'CEO einer Firma wird verhaftet', context: 'Das Unternehmen verliert 60% in einem Tag', answer: 'specific' },
  { emoji: '📉', title: 'Globale Finanzkrise 2008', context: 'Alle Märkte weltweit brechen ein', answer: 'systematic' },
  { emoji: '🚗', title: 'Grosser Produktrückruf bei einem Autohersteller', context: 'Millionen Autos müssen zurück', answer: 'specific' },
  { emoji: '🦠', title: 'Weltweite Pandemie bricht aus', context: 'Lockdowns auf allen Kontinenten', answer: 'systematic' },
  { emoji: '📊', title: 'Buchhaltungsskandal bei einem Konzern', context: 'Jahrelange Bilanzfälschung aufgedeckt', answer: 'specific' },
  { emoji: '🏦', title: 'Notenbanken erhöhen Zinsen weltweit', context: 'Kapitalkosten steigen überall gleichzeitig', answer: 'systematic' },
];

/* ── Correlation data ── */
const assets = ['S&P 500', 'NASDAQ', 'MSCI EM', 'Anleihen', 'Gold'];
...
/* ── Quiz ── */
const quiz = {
  question: 'MSCI World hält 1.500 Firmen aus 23 Ländern und fiel im März 2020 trotzdem um 34%. Was lehrt uns das?',
  answers: [
    { id: 'a', text: 'Der ETF hat seinen Index nicht korrekt abgebildet' },
    { id: 'b', text: 'Mehr ETFs hätten den Verlust verhindert' },
    { id: 'c', text: 'Selbst maximale Aktien-Diversifikation schützt nicht vor einem globalen systematischen Schock' },
    { id: 'd', text: 'ETFs sind in allen Märkten sicherer als Einzelaktien' },
  ],
  correctId: 'c',
  correctFeedback: 'Genau. MSCI World = maximale Aktien-Diversifikation. Trotzdem -34% weil COVID alle Aktienmärkte gleichzeitig traf. Nur unkorrelierte Assets (Anleihen, Gold) boten teilweise Schutz.',
  wrongFeedback: 'MSCI World = maximale Streuung in Aktien. Trotzdem -34% im COVID-Crash. Systematische Schocks treffen alles gleichzeitig — nur wirklich andere Anlageklassen helfen dann.',
};
      ...
      <AnimatePresence mode="wait">
            ...
            <AnimatePresence mode="wait">
              ...
              {storySlide === 0 && (
                <motion.div key="ss0" className="flex flex-col items-center max-w-sm mx-auto w-full" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.3 }}>
                  ...
                  <h2 className="font-display text-2xl font-bold text-foreground text-center mb-5 leading-tight">10 Playlists, alle von Taylor Swift.</h2>
                  <div className="w-full rounded-2xl p-5 mb-4" style={{ backgroundColor: '#1E3A5F' }}>
                    <p className="font-body text-[10px] text-white/60 mb-3">Dein Portfolio</p>
                    ...
                    <div className="border-t border-white/20 pt-3">
                      {!scandalRevealed ? (
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }}>
                          <p className="font-body text-xs text-red-300 text-center">⚡ Taylor Swift Skandal</p>
                          <p className="font-body text-[10px] text-red-300/70 text-center mt-1">Alle 5 Playlists — gleichzeitig leer</p>
                        </motion.div>
                      )}
                    </div>
                  </div>
                  <p className="font-body text-sm text-muted-foreground text-center mb-4">5 ETFs, alle US-Tech = dasselbe Problem</p>
              ...
              {storySlide === 1 && (
                <motion.div key="ss1" className="flex flex-col items-center max-w-sm mx-auto w-full" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.3 }}>
                  <h2 className="font-display text-xl font-bold text-foreground text-center mb-5 leading-tight">ETFs schützen vor einer Sache — nicht vor allem.</h2>
                  <div className="grid grid-cols-2 gap-3 w-full mb-4">
                    <div className="rounded-2xl p-4 flex flex-col items-center text-center" style={{ backgroundColor: '#ECFDF5' }}>
                      <span className="text-3xl mb-2">✅</span>
                      <p className="font-display text-sm font-bold text-foreground mb-1">Spezifisches Risiko</p>
                      <p className="font-body text-xs text-foreground/80 mb-1">Eine Firma geht pleite</p>
                      <p className="font-body text-[10px] text-muted-foreground mb-2">Wirecard-Skandal 2020</p>
                      <span className="inline-block px-2 py-0.5 rounded-full bg-green-500/20 text-green-700 font-body text-[10px] font-semibold">ETF schützt dich ✓</span>
                    </div>
                    <div className="rounded-2xl p-4 flex flex-col items-center text-center" style={{ backgroundColor: '#FEF2F2' }}>
                      <span className="text-3xl mb-2">⚠️</span>
                      <p className="font-display text-sm font-bold text-foreground mb-1">Systematisches Risiko</p>
                      <p className="font-body text-xs text-foreground/80 mb-1">Der gesamte Markt crasht</p>
                      <p className="font-body text-[10px] text-muted-foreground mb-2">COVID März 2020: -34%</p>
                      <span className="inline-block px-2 py-0.5 rounded-full bg-red-500/20 text-red-700 font-body text-[10px] font-semibold">ETF schützt NICHT ✗</span>
                    </div>
                  </div>
                  <p className="font-body text-xs text-muted-foreground text-center mb-4">MSCI World — 1.500 Firmen, 23 Länder — fiel trotzdem 34% in 6 Wochen.</p>
              ...
              {storySlide === 2 && (
                <motion.div key="ss2" className="flex flex-col items-center max-w-sm mx-auto w-full" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.3 }}>
                  <h2 className="font-display text-xl font-bold text-foreground text-center mb-5 leading-tight">Echte Diversifikation braucht verschiedene Welten.</h2>
                  <div className="grid grid-cols-2 gap-3 w-full mb-4">
                    <div className="rounded-2xl p-4 flex flex-col items-center text-center" style={{ backgroundColor: '#FEF2F2' }}>
                      <span className="text-3xl mb-2">❌</span>
                      <p className="font-display text-sm font-bold text-foreground mb-2">Schein-Diversifikation</p>
                      ...
                      <p className="font-body text-[10px] text-red-600">5 Playlists, 1 Künstler</p>
                    </div>
                    <div className="rounded-2xl p-4 flex flex-col items-center text-center" style={{ backgroundColor: '#ECFDF5' }}>
                      <span className="text-3xl mb-2">✅</span>
                      <p className="font-display text-sm font-bold text-foreground mb-2">Echte Diversifikation</p>
                      <div className="flex flex-wrap gap-1 justify-center mb-2">
                        {[
                      ...
                      <p className="font-body text-[10px] text-green-600">4 verschiedene Welten</p>
                    </div>
                  </div>
                  <div className="w-full rounded-xl bg-amber-500/10 border border-amber-500/20 p-3 mb-2">
                    <p className="font-body text-xs text-amber-700 text-center">💡 Mehr ETFs ≠ mehr Sicherheit. Es kommt auf die Korrelation an.</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
        ...
        {currentStep === 1 && (
          <motion.div key="s1" className="flex-1 flex flex-col px-6 py-4 overflow-y-auto" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
            <h2 className="font-display text-xl font-bold text-foreground text-center mb-1">Spezifisch oder Systematisch?</h2>
            <p className="font-body text-xs text-muted-foreground text-center mb-4">Tippe die richtige Kategorie für jedes Ereignis</p>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-sm mx-auto w-full">
                <div className={`rounded-2xl p-5 text-center mb-4 ${sortScore >= 6 ? 'bg-green-500/10 border border-green-500/20' : sortScore >= 4 ? 'bg-green-500/10 border border-green-500/20' : 'bg-amber-500/10 border border-amber-500/20'}`}>
                  <p className="font-display text-lg font-bold text-foreground mb-2">
                    {sortScore >= 6 ? '🎯 Perfekt! Du erkennst beide Risikoarten.' : sortScore >= 4 ? '👍 Gut! Fast alle richtig.' : '💪 Üben macht den Meister!'}
                  </p>
                  <p className="font-display text-2xl font-bold text-foreground mb-3">{sortScore}/{sortEvents.length}</p>
                </div>
                <div className="rounded-2xl bg-card border border-border p-4">
                  <p className="font-body text-xs text-foreground mb-1"><strong>Spezifisch</strong> = ETF schützt ✓</p>
                  <p className="font-body text-xs text-foreground"><strong>Systematisch</strong> = niemand schützt ✗</p>
                </div>
              </motion.div>
            )}
          </motion.div>
        ...
        {currentStep === 2 && (
          <motion.div key="s2" className="flex-1 flex flex-col px-6 py-4 overflow-y-auto" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
            <h2 className="font-display text-xl font-bold text-foreground text-center mb-1">Wie bewegen sich Assets zusammen?</h2>
            <p className="font-body text-xs text-muted-foreground text-center mb-4">Wechsle zwischen Marktphasen</p>

            <div className="flex rounded-full bg-muted p-1 max-w-sm mx-auto w-full mb-4">
              {([
                { key: 'normal' as MarketPhase, label: '📈 Normal' },
                { key: 'crisis2008' as MarketPhase, label: '💥 Krise 2008' },
                { key: 'crisis2020' as MarketPhase, label: '🦠 Krise 2020' },
              ]).map(m => (
                <button
                  key={m.key}
            ...
            <div className="flex gap-3 justify-center mb-3">
              <span className="font-body text-[10px] text-muted-foreground">🟢 Tief = gut für Diversifikation</span>
              <span className="font-body text-[10px] text-muted-foreground">🔴 Hoch = bewegt sich zusammen</span>
            </div>
            ...
            <div className={`max-w-sm mx-auto w-full rounded-xl p-3 ${activeMarket === 'normal' ? 'bg-blue-500/10 border border-blue-500/20' : 'bg-amber-500/10 border border-amber-500/20'}`}>
              <p className="font-body text-xs text-foreground text-center">
                {activeMarket === 'normal'
                  ? 'Anleihen und Gold bewegen sich oft entgegengesetzt zu Aktien — gute Puffer.'
                  : '⚠️ Siehst du wie alles roter wird? In Krisen fallen fast alle Aktien-Assets gleichzeitig. Genau dann versagt Diversifikation am stärksten.'}
              </p>
            </div>
          </motion.div>
        )}
        ...
        {currentStep === 4 && (
          <motion.div key="s4" className="flex-1 flex flex-col items-center justify-center px-6 py-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>
            ...
            <div className="w-full max-w-sm rounded-2xl bg-green-500/10 border border-green-500/20 p-4 mb-4 space-y-2">
              <p className="font-body text-xs text-foreground">✅ Spezifisch = eine Firma, durch ETFs eliminierbar</p>
              <p className="font-body text-xs text-foreground">✅ Systematisch = Gesamtmarkt, bleibt immer</p>
              <p className="font-body text-xs text-foreground">✅ 5 US-ETFs = nicht diversifiziert (gleicher Künstler)</p>
              <p className="font-body text-xs text-foreground">✅ Korrelationen steigen in Krisen — genau dann gefährlich</p>
            </div>
          ...
          <motion.div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowDeepDive(false)}>
            <motion.div className="bg-card rounded-t-3xl p-6 max-w-md w-full max-h-[70vh] overflow-y-auto" initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} onClick={e => e.stopPropagation()}>
              ...
              <p className="font-body text-sm text-muted-foreground leading-relaxed mb-3">
                Modern Portfolio Theory (Markowitz, 1952): Unsystematisches Risiko sinkt schnell mit mehr Titeln. Bei ~20 unkorrelierten Aktien nähert es sich null. Was bleibt ist Beta — Marktsensitivität.
              </p>
              <p className="font-body text-sm text-muted-foreground leading-relaxed">
                Korrelationsinstabilität: In Krisenzeiten konvergieren Korrelationen gegen 1.0 — der Diversifikationsvorteil erodiert genau dann wenn er am meisten gebraucht wird.
              </p>
