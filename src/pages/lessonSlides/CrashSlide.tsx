...
const sectors = [
  { icon: '💻', label: 'Technologie', base: 2000 },
  { icon: '🏥', label: 'Gesundheit', base: 2000 },
  { icon: '⚡', label: 'Energie', base: 2000 },
  { icon: '🛒', label: 'Konsum', base: 2000 },
  { icon: '🏦', label: 'Finanzen', base: 2000 },
];
...
const CrashSlide = ({ onComplete }: { onComplete: () => void }) => {
  ...
  const crashedValue = 1200; // 2000 * 0.6
  const totalAfter = 4 * 2000 + crashedValue; // 9200
  ...
  return (
    ...
    >
      <h2 className="font-display text-xl font-bold text-foreground text-center mt-2 mb-5">
        Was passiert wenn eine Branche crasht?
      </h2>
              ...
              <div className="bg-card border border-border rounded-2xl p-4 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <p className="font-body text-sm text-foreground">
                    Alles in eine Aktie: <span className="font-bold text-red-600 dark:text-red-400">-40% → CHF 6'000</span>
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-green-500 flex-shrink-0" />
                  <p className="font-body text-sm text-foreground">
                    Verteilt auf 5 Branchen: <span className="font-bold text-green-600 dark:text-green-400">-8% → CHF 9'200</span>
                  </p>
                </div>
              </div>
