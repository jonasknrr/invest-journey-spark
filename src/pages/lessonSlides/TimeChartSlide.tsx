...
const datasets: Record<string, { data: number[]; end: number; label: string; color?: string }> = {
  '1y': { data: data1y, end: 9500, label: 'Short-term: unpredictable' },
  '5y': { data: data5y, end: 16000, label: 'Medium-term: a trend emerges' },
  '20y': { data: data20y, end: 45000, label: 'Long-term: time works for you', color: 'hsl(142, 71%, 45%)' },
};
  ...
  return (
    ...
    >
      <h2 className="font-display text-xl font-bold text-foreground text-center mt-2 mb-5">
        See for yourself what time does
      </h2>
      ...
      <div className="flex-1 flex flex-col items-center justify-center max-w-sm mx-auto w-full">
        ...
        {!allVisited && (
          <p className="font-body text-xs text-muted-foreground/60 text-center mt-4">
            Tap all three timeframes to continue
          </p>
        )}
      </div>
    </motion.div>
